...
private:
    ...
    TagElementsSet*     m_pElementsSetList; // 对象池数组
    int			        m_nNumOfAlloc; // 使用数量
    TagElement*	        m_pUnusedElementsList; // 当前可用的节点
    int			        m_nNumOfElements; // 一个数组的最大对象数量也是内存池的初始大小
    int			        m_nNumOfElementsSet; // 数组数量
